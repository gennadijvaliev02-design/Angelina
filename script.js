...
let slideIndex=0, autoTimer=null, userPauseTimer=null;
function goToSlide(index,behavior='smooth'){
  if(!carousel||!slides.length)return;
  slideIndex=(index+slides.length)%slides.length;
  carousel.scrollTo({left:slides[slideIndex].offsetLeft,behavior});
  [...dotsRoot.children].forEach((d,i)=>d.classList.toggle('active',i===slideIndex));
}
slides.forEach((_,i)=>{const d=document.createElement('button');d.className='carousel-dot'+(i===0?' active':'');d.type='button';d.setAttribute('aria-label',`Трансформация ${i+1}`);d.addEventListener('click',()=>{pauseAutoplay();goToSlide(i)});dotsRoot?.appendChild(d);});
...
function startAutoplay(){clearInterval(autoTimer);if(window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;autoTimer=setInterval(()=>goToSlide(slideIndex+1),5500);}
function pauseAutoplay(){clearInterval(autoTimer);clearTimeout(userPauseTimer);userPauseTimer=setTimeout(startAutoplay,9000);}
carousel?.addEventListener('pointerdown',pauseAutoplay,{passive:true});
carousel?.addEventListener('scroll',()=>{if(!carousel||!slides.length)return;const center=carousel.scrollLeft+carousel.clientWidth/2;let best=0,dist=Infinity;slides.forEach((s,i)=>{const d=Math.abs((s.offsetLeft+s.offsetWidth/2)-center);if(d<dist){dist=d;best=i;}});if(best!==slideIndex){slideIndex=best;[...dotsRoot.children].forEach((d,i)=>d.classList.toggle('active',i===slideIndex));}},{passive:true});
startAutoplay();
